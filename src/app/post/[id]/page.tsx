...
import tikTokLogoSmall from "@/assets/images/tiktok-logo-small.png";
import {
...
function PostPage() {
  ...
  const [createComment, { data: commentData }] = useCreateCommentMutation({
    refetchQueries: [
      {
  ...
  const { data, loading: loadingComments } = useGetCommentsByPostIdQuery({
    variables: {
      postId: Number(id),
    },
  });
  ...
  const loopThroughPostsUp = () => {
    if (
      currentPostIdIndex ===
      (dataPost?.getPostById?.otherPostIds?.length ?? -1) - 1
    )
      return;
    ...
    router.replace(`/post/${nextPostId}`);
  };
  ...
  const { data: dataPost, loading: loadingPost } = useGetPostByIdQuery({
    variables: {
      id: Number(id),
    },
    onCompleted: () => {
  ...
  const [removeLikeMutation] = useUnlikePostMutation({
    variables: {
      postId: Number(id),
    },
    refetchQueries: [
  ...
  const isLiked = likedPosts.some((likedPost: { userId: number }) => {
      ...
      <div className="lg:w-[calc(100%-540px)] h-full relative">
        ...
        {loadingPost ? (
          ...
          >
            <video
              ref={video}
              src={`${server}/${dataPost?.getPostById.video}`}
              loop
              muted
